...
- [ ] NOT done: pending "New from Granola: <name> — add / dismiss" on `/dating`. No existing table/JSON store fits cleanly (User has no settings JSON; PersonalRecord.data is PII for /personal; DatingEvent needs a person). Unmatched people are only returned in the endpoint's `suggestions` (not persisted). Needs a decision: a small `DatingSuggestion` table, or a JSON column on User
...
- [ ] Eddie: prod `pnpm db:push` (2 nullable columns + 1 unique index on DatingEvent) BEFORE deploying; point the Granola routine at `POST /api/capture/dating/notes` with `Authorization: Bearer $CAPTURE_TOKEN`
