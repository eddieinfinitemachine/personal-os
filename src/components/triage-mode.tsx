...
type Picker = "project" | "list" | "date" | null;

export function TriageLauncher({ projectId }: { projectId: string }) {
  ...
  return (
    <>
      ...
      {open ? (
        <TriageMode projectId={projectId} onClose={() => setOpen(false)} />
      ) : null}
    </>
  );
}
...
function TriageMode({
    ...
    (async () => {
      try {
        const [tRes, pRes, lRes] = await Promise.all([
          fetch(`/api/todos?projectId=${projectId}`),
          fetch("/api/projects"),
          fetch("/api/lists"),
        ]);
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [picker, queue.length, current, complete, remove, undo, close]);
      ...
      {current ? (
        <div className="px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]">
          ...
          <p className="mt-3 hidden md:block text-center text-xs text-[var(--color-muted-foreground)]">
            j / k next · prev — actions advance automatically
          </p>
        </div>
      ) : null}
